...
export async function GET(request: NextRequest) {
  ...
  if (CRON_SECRET && CRON_SECRET !== "disabled" && providedSecret !== CRON_SECRET) {
    return NextResponse.json({ error: "Unauthorized", debug: { hasCronSecret: !!CRON_SECRET, provided: providedSecret.slice(0, 5) + "..." } }, { status: 401 });
  }
